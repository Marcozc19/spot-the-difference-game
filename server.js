...
app.get('/api/querypic', async (req, res) => {
  ...
  try {
    const response = await axios.get(`https://api.echo3D.com/query?key=${apiKey}&secKey=${secKey}&filters=sdf,${set},pic`);
...
app.get('/api/queryasset', async (req, res) => {
  const set = req.query.set; // Set name should be passed as a query parameter
  try {
    const response = await axios.get(`https://api.echo3D.com/query?key=${apiKey}&secKey=${secKey}&filters=sdf,${set},asset`);
    res.json(response.data);
  } catch (error) {
    console.error('Error fetching data from echo3D API:', error);
    res.status(500).json({ error: 'Failed to fetch data from echo3D API' });
...
app.get('/api/entrydata', async (req, res) => {
  ...
  try {
    const response = await axios.get(`https://api.echo3D.com/query?key=${apiKey}&secKey=${secKey}&entry=${entryId}`);
    res.json(response.data);
  } catch (error) {
    console.error('Error fetching data from echo3D API:', error);
    res.status(500).json({ error: 'Failed to fetch data from echo3D API' });
