...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  async function loadAsset() {
    try {
      ...
      const assets = Object.keys(data.db).map((key) => {
        ...
        if (
          key &&
          additionalData.qrWebARStorageID &&
          additionalData.x &&
          additionalData.y &&
  ...
  async function loadSet() {
    ...
    try {
      ...
      const pics = Object.keys(data.db).map(
        (key) => data.db[key].additionalData.compressedImageStorageID
      );
      if (pics.length === 2) {
  ...
  document
    ...
    .addEventListener("click", (event) => handleImageClick(event));
  document
    .getElementById("image2")
    .addEventListener("click", (event) => handleImageClick(event));
  document
    .getElementById("next-button")
    .addEventListener("click", checkDifferencesAndRedirect);
  document
    .getElementById("back-button")
    .addEventListener("click", backToLastSet);
});
